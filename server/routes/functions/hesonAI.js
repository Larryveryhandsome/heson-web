...
const { routeChat } = require('../../utils/claude');
...
當你已收集到全部6項資訊後，請在回覆末尾加上以下 JSON 標記：
...
router.post('/', async (req, res) => {
  try {
    ...
    const { reply: rawReply, model, tokens } = await routeChat(SYSTEM_PROMPT, message, historyText);
    ...
    res.json({ reply, booking_data, _meta: { model, tokens } });
  } catch (error) {
    console.error('[hesonAI]', error.message);
